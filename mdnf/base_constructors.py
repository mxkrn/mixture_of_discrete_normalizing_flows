...
ZERO_PROBABILITY = 1e-4


def yield_probabilities_delta_diagonal(N, K):
    while True:
        for k in range(K):        
            probs = np.ones( (N, K) ) * ZERO_PROBABILITY
            probs[:, k] = 1.
            probs /= np.sum(probs, -1, keepdims=True)
            yield probs        


def yield_probabilities_delta_random(N, K):
    while True:
        probs = np.ones( (N,K) ) * ZERO_PROBABILITY
...
def yield_probabilities_delta(N, K, ):
    if K**N>1000000:
        ...
        diagonal = yield_probabilities_delta_diagonal(N, K)
        for k in range(K):        
            yield next(diagonal)

        random = yield_probabilities_delta_random(N, K)
        while True:
            yield next(random)    
...
def sample_probabilities_from_dirichlet(N, K, alpha):
    logger.debug("[sample_probabilities_from_dirichlet] N=%s K=%s alpha=%s" % (N, K, alpha) )
    probs = np.random.dirichlet(np.ones(K)*alpha, N) + ZERO_PROBABILITY
    probs /= np.sum(probs, -1, keepdims=True)
    return probs  
...
def create_base_mixture(N, K, B=None, specification="p", independent_variables=False,
    ...
    peak = yield_probabilities_delta(N, K)
    diagonal = yield_probabilities_delta_diagonal(N, K)
    peak_random = yield_probabilities_delta_random(N, K)
    ...
    while len(components)<B:
        ...
        if component_type=='u': 
            ...
            component = MultivariateFactorizedCategorical( probs=sample_probabilities_from_dirichlet(N, K, alpha) )
        else:
